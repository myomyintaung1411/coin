<template>
    <section class="w-full h-full pb-5 relative">
        <div class="pt-7 px-5">
            <span class="text-2xl tracking-wide font-bold">手机号登录</span>
        </div>
        <form @click.prevent>
          <div class="py-5 px-5 relative w-full">
                <span class="text-sm text-[#7bc3e7]">手机号</span>
            <div class=" mt-4 mb-4 h-12 not_round w-full text-base flex items-center  active_btn ">
                <input  v-model.trim="phoneNumber" type="tel"  placeholder=" 请输入手机号码" autocomplete="off"     maxlength="11" class="w-full h-full bg-transparent px-3  border-none outline-none outline-0 focus:outline-none">
            </div> 
                <span class="text-sm text-[#7bc3e7]">密码</span>
            <div class=" mt-4 mb-4 h-12 not_round w-full text-base flex items-center  active_btn ">
                <input v-model.trim="password"   placeholder=" 请输入密码" autocomplete="off"  type="password"   maxlength="16" class="w-full h-full bg-transparent px-3  border-none outline-none outline-0 focus:outline-none">
            </div> 
            ...
            <div class=" mt-4 mb-4 h-12 not_round w-full text-base flex items-center  active_btn ">
             ...
             <!-- <div class="absolute right-6 cursor-pointer">
               ...
               <div class="absolute right-6 cursor-pointer">
               <button v-if="!svgCodeData" @click="getSvgCode()" class=" whitespace-nowrap mr-4 px-3 py-1 text-white rounded tracking-wide text-sm bg-[#41a7d8]">验证码</button>
            ...
            <div class="pt-5  relative w-full">
                <van-button @click="goLogin" :loading="loading" :disabled="loading" block type="primary" color="#41a7d8">立即登录</van-button>
            </div>
            <div class="pt-5  relative w-full">
                <div @click="goReg"  class="shadow_color py-3 w-full h-full text-[#41a7d8] flex items-center justify-center" >
                立即注册
                </div>
                <!-- <van-button block type="primary" color="#41a7d8">立即注册</van-button> -->
            </div>
         </div>
        </form>
        <div @click="goForget" class=" flex justify-end px-5 text-[#41a7d8]">
            忘记密码
        </div>
  ...
  setup() {
    ...
    onMounted(() => {
      //state.imgCode = draw();
        ...
        phoneNumber.value = route.query?.acc;
       }
    });
    ...
    const goLogin = () => {
    if(phoneNumber.value == '' || password.value == '' ||  recaptcha.value == '' 

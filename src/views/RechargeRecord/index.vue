<template>
  <section class="w-full h-screen relative text-[#303437] overflow-y-auto">
    <div class="h-14 flex items-center justify-between px-3">
      ...
      <span class="text-base tracking-wide w-1/3 text-center"> 充值记录 </span>
      <span class="text-xs tracking-wide w-1/3"></span>
    </div>
    <section class="py-5">
      <div v-if="rechargeData?.length !== 0" class="flex flex-col justify-center items-center pt-20">
...
const getRechargeReocrd = async () => {
  let data = {
    pageSize: pageSize.value,
    currentPage: currentPage.value,
  };
  try {
    const res = await allApi.RechargeInfoApi(data);
    ...
    if (res?.data?.success && res?.data?.code == 200) {
      ...
      if (rechargeData.value?.length >= parseInt(res?.data?.data?.count)) {
        finished.value = true;
      }
      //rechargeData.value = res?.data?.data?.record;
    }else{
      loading.value = false

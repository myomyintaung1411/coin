<template>
    ...
    <div class="h-14 flex items-center justify-between px-3">
      ...
      <span class="text-base tracking-wide w-1/3 text-center"> 资金明细 </span>
    ...
    <section class="py-4 px-3 w-full relative">
        <div v-for="fund in fundListData" :key="fund?.id" class="py-2 text-sm">
            <div class="un_active px-3 py-2 not_round">
                <div class="text-base font-bold tracking-wide py-2">
                    【{{fund?.type}}】{{fund?.content}}{{fund?.transaction_amount}}元
                </div>
                <div class="flex items-center px-2 justify-between text-[#a3a7aa]">
                    <span>{{fund?.create_time}}</span>
                    <span class="text-green-500">+{{fund?.transaction_amount}}</span>
                </div>
            </div>
        </div>
    </section>
...
const getFundDetails = async () => {
  ...
  try {
    const res = await allApi.FundDetailsApi(data);
    ...
    if (res?.data?.success && res?.data?.code == 200) {
      ...
      if (fundListData.value?.length >= parseInt(res?.data?.data?.count)) {
        finished.value = true;
      }
      //fundListData.value = res?.data?.data?.record;
    }else{
      loading.value = false

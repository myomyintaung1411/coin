<template>
    ...
    <div class="h-14 flex items-center justify-between px-3">
      ...
      <span class="text-base tracking-wide w-1/3 text-center"> 系统增值宝 </span>
...
const system_yebao = ref(null)

const SystemYueBao = async () => {
    try {
        const res = await allApi.SystemYueBaoApi();
		console.log(res.data,"SystemYueBaoApi")
         //showToast(res?.data?.msg);
        if (res?.data?.success && res?.data?.code == 200) {
          system_yebao.value = res.data?.data
